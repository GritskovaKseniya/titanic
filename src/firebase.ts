...
// gated by firestore.rules instead. Images live as plain files in public/,
// not Firebase Storage, so no storage bucket or billing plan is needed.
...
// live content but never signs in. Auth is pulled in by src/admin/* (getAuth
// against this same `app`), which is loaded as its own chunk only when
// someone visits /admin.
